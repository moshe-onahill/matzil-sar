...
export default function UserProfilePage({
  ...
  const isAdmin = currentRole === "Global Admin" || currentRole === "SAR Manager" || currentRole === "Dispatcher";
      ...
      <div className="mx-auto max-w-3xl space-y-4">
        ...
        <div className="rounded-xl bg-gray-900 p-5 space-y-3">
          {!editMode ? (
            ...
            <>
              <input value={fullName} onChange={(e) => setFullName(e.target.value)} className="w-full rounded bg-black px-3 py-2" placeholder="Full Name" />
              <input value={callSign} onChange={(e) => setCallSign(e.target.value)} className="w-full rounded bg-black px-3 py-2" placeholder="Call Sign" />
              <input value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full rounded bg-black px-3 py-2" placeholder="Phone" />
              <div>Email: {user.email}</div>
              ...
              <div className="space-y-2">
                ...
                {allRoles.map((r) => (
                  <label key={r.id} className="flex items-center gap-2">
                    <input type="checkbox" checked={selectedRoles.includes(r.id)} onChange={() => toggle(selectedRoles, r.id, setSelectedRoles)} />
                    <span>{r.name}</span>
                  </label>
                ))}
              </div>
              ...
              <div className="space-y-2">
                ...
                {allUnits.map((u) => (
                  <label key={u.id} className="flex items-center gap-2">
                    <input type="checkbox" checked={selectedUnits.includes(u.id)} onChange={() => toggle(selectedUnits, u.id, setSelectedUnits)} />
                    <span>{u.name}</span>
                  </label>
                ))}
              </div>
          ...
          {isAdmin && (
            <div className="mt-3 flex flex-wrap gap-2">
              {!editMode ? (
                <button onClick={() => setEditMode(true)} className="rounded bg-gray-700 px-4 py-2">Edit</button>
              ) : (
                <>
                  <button onClick={() => void saveUser()} className="rounded bg-green-600 px-4 py-2">Save</button>
                  <button
                    onClick={() => {
                      setEditMode(false);
              ...
              {isDeactivated ? (
                <button onClick={() => void reactivate()} className="rounded bg-green-700 px-4 py-2">Reactivate</button>
              ) : (
                <button onClick={() => void deactivate()} className="rounded bg-red-700 px-4 py-2">Deactivate</button>
              )}

              <button onClick={() => void deleteUser()} className="rounded bg-red-900 px-4 py-2">Delete</button>
              <button onClick={() => void resendInvite()} className="rounded bg-blue-600 px-4 py-2">Resend Invite</button>
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl bg-gray-900 p-5">
          ...
          <div className="space-y-2">
            {activity.map((a, i) => (
              <div key={i} className="rounded bg-black/30 px-3 py-2 text-sm">
                <div>{a.text}</div>
