...
export async function POST(req: Request) {
  ...
  if (body.broadcast_id) {
    ({ error } = await supabaseAdmin.from("notification_logs").update({ status: "closed" }).eq("broadcast_id", body.broadcast_id));
  } else if (body.id) {
    ({ error } = await supabaseAdmin.from("notification_logs").update({ status: "closed" }).eq("id", body.id));
  } else {
