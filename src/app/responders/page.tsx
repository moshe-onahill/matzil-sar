    ...
    <main className="min-h-screen bg-black px-4 py-5 pb-28 text-white sm:p-6 sm:pb-28">
      <div className="mx-auto max-w-5xl space-y-4">
        ...
        <div className="space-y-2">
          ...
          {filtered.map((row) => (
            <div key={row.id} className="flex items-center gap-3 rounded-lg bg-gray-900 px-3 py-2">
              {/* Left: identity */}
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-semibold">{row.call_sign}</span>
                  <span className="text-xs text-gray-400">{row.full_name}</span>
                  <span className={`rounded px-1.5 py-0.5 text-xs ${row.response_type === "On Location" ? "bg-green-950/60 text-green-300" : "bg-red-950/50 text-red-200"}`}>
                    {formatEta(row)}
                  </span>
                </div>
                {row.incident && (
                  <Link href={`/incidents/${row.incident.id}`} className="mt-0.5 flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-300 truncate">
                    <span className="font-mono">{row.incident.incident_number}</span>
